...
function resolveTypeImage(code, cb) {
    const candidates = [
        `img/${code}.png`,
        `img/${code}.jpg`,
        `img/${code}.jpeg`,
    ...
    let i = 0;
    function tryNext() {
        if (i >= candidates.length) return cb(candidates[candidates.length - 1]);
...
function renderQuestions() {
    ...
    visibleQuestions.forEach((q, index) => {
        ...
        metaLabel.textContent = getQuestionMetaLabel(q);
        ...
        // options (build DOM safely to avoid HTML injection)
        ...
        q.options.forEach((opt, i) => {
            ...
            if (app.answers[q.id] === opt.value) input.checked = true;

            const codeDiv = document.createElement('div');
            codeDiv.className = 'option-code';
...
function renderResult() {
    ...
    document.getElementById('typeName').textContent = `${type.code}（${type.cn}）`;
    document.getElementById('typeIntro').textContent = type.intro;
    ...
    const explicit = TYPE_IMAGES[type.code];
    if (explicit) {
        typeImage.src = explicit;
        typeImage.alt = `${type.code}（${type.cn}）`;
        posterBox.classList.remove('no-image');
    } else {
        // try common file extensions and fallback to share.png
        resolveTypeImage(type.code, (url) => {
            if (url) {
                typeImage.src = url;
                typeImage.alt = `${type.code}（${type.cn}）`;
                posterBox.classList.remove('no-image');
...
function copyResult() {
    const result = computeResult();
    const type = result.finalType;
    const text = `【SBTI人格测试】我的人格类型是：${type.code}（${type.cn}）\n\n${type.intro}\n\n—— SBTI人格测试 https://sbti.jmkj.xyz/`;
...
function shareResult() {
    const result = computeResult();
    const type = result.finalType;
    const title = `我的SBTI人格是：${type.code}（${type.cn}）`;
...
function showWechatPopup() {
    ...
    const card = document.getElementById('wechatPopupCard');
    const closeBtn = document.getElementById('wechatPopupClose');
    popup.style.display = 'flex';
    popup.setAttribute('aria-hidden', 'false');
    // focus management
    if (closeBtn) closeBtn.focus();
    // add escape handler
    document.addEventListener('keydown', handleWechatEscape);
}

function closeWechatPopup() {
    const popup = document.getElementById('wechatPopup');
    const floatBtn = document.getElementById('wechatFloatBtn');
    popup.style.display = 'none';
    popup.setAttribute('aria-hidden', 'true');
    localStorage.setItem('wechat_popup_closed', Date.now());
    // return focus
    if (floatBtn) floatBtn.focus();
    // remove escape handler
    document.removeEventListener('keydown', handleWechatEscape);
}

function handleWechatEscape(e) {
...
function initWechatFeature() {
    if (!isWechatBrowser()) return;

    const floatBtn = document.getElementById('wechatFloatBtn');
    floatBtn.classList.add('show');

    const closed = localStorage.getItem('wechat_popup_closed');
    const now = Date.now();
    const oneDay = 24 * 60 * 60 * 1000;

    if (!closed || (now - parseInt(closed) > oneDay)) {
        setTimeout(() => {
            showWechatPopup();
        }, 3000);
    }
...
document.addEventListener('DOMContentLoaded', initWechatFeature);
